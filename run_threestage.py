...
from common.threestage_model import *
...
if not args.disable_optimizations and not args.dense and args.stride == 1:
    ...
    model_pos_train = Three_Stage_Model(poses_valid_2d[0].shape[-2], poses_valid_2d[0].shape[-1],
...
model_pos = Three_Stage_Model(poses_valid_2d[0].shape[-2], poses_valid_2d[0].shape[-1], dataset.skeleton().num_joints(),
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        for batch_cam, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            # Predict 3D poses
            predicted_3d_pos = model_pos_train(inputs_2d, inputs_param[:, :4])
        ...
        with torch.no_grad():
            ...
            if not args.no_eval:
                ...
                for cam, batch, batch_2d in test_generator.next_epoch():
                    ...
                    # Predict 3D poses
                    predicted_3d_pos = model_pos(inputs_2d, inputs_param[:, :4])
                    loss_3d_pos = mpjpe(predicted_3d_pos, inputs_3d)
        ...
        if args.no_eval:
            ...
            log_path = os.path.join(args.checkpoint, 'Three_stage_training_log.txt')
        ...
        if epoch % args.checkpoint_frequency == 0:
            chk_path = os.path.join(args.checkpoint, 'Three_stage_' + str(args.stage) + '_epoch_{}.bin'.format(epoch))
            ...
            torch.save({
                ...
                # 'current_S2_loss': losses_3d_valid_S2[-1] * 1000,
                # 'current_all_loss': losses_3d_valid_all[-1] * 1000,
            }, chk_path)


        #### save best checkpoint
        best_chk_path = os.path.join(args.checkpoint, 'Three_stage_' + str(args.stage) + '_best_epoch.bin'.format(epoch))
...
def evaluate(test_generator, action=None, return_predictions=False):
    ...
    with torch.no_grad():
        ...
        if args.test_time_augmentation:
            for batch_cam_no_flip, batch_cam, batch, batch_2d, batch_2d_flip in test_generator.next_epoch():
                ...
                # Positional model
                predicted_3d_pos = model_pos(inputs_2d, inputs_param_no_flip[:, :4])
                predicted_3d_pos_flip = model_pos(inputs_2d_flip, inputs_param[:, :4])
    ...
    log_path = os.path.join(args.checkpoint, 'Three_stage_evaluating_log.txt')
    f = open(log_path, mode='a')
    if action is None:
        print('----------')
        f.write('----------\n')
...
if args.render:
    ...
    def fetch_actions(actions):
        ...
        for subject, action in actions:
            ...
            for i in range(len(poses_2d)):  # Iterate across cameras
                out_poses_2d.append(poses_2d[i])
                camera = dataset._cameras[subject][i]['intrinsic']
